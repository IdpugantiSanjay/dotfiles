...
function ls --description "List contents of directory"
    ...
    if not set -q __fish_ls_command
        ...
        if command -sq colorls
            ...
            # Check if ls supports the hyperlink option
            if command ls --hyperlink=auto / >/dev/null 2>/dev/null
                set -g __fish_ls_hyperlink_opt --hyperlink=auto
            end
        end
    end
    ...
    command $__fish_ls_command $__fish_ls_color_opt $__fish_ls_hyperlink_opt $indicators_opt $argv
end
