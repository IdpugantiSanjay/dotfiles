...
function backup -d 'backup script using rsync' -a source_dir destination_dir
	...
	rsync -a --backup --delete --recursive --verbose --backup-dir="$destination_dir/$timestamp" $source_dir $destination_dir >> "/var/log/backup/$timestamp-stdout.txt" 2>> "/var/log/backup/$timestamp-stderr.txt"
end
